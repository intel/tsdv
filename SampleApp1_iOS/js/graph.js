...
var tsdvjs = (function(args) {
    ...
    */
    ...
    TSDVJS.LOG = function(startTime, dataLength, method) {
        if (TSDVJS.loggingOptions.logToFile) //Possibly add a check to only log if it takes more than 0 time?
        {
            ...
            TSDV.logToFile(sqlFormatSec(currDate), currDate.getTime() - startTime, dataLength, method);
        }
    }
    ...
    TSDVJS.Graph = function(args) {
        ...
        this.yAxisLeft = args.yAxisLeft;
        this.yAxisRight = args.yAxisRight;
        this.visibleData = args.visibleData;
        this.drawGraph = function() {
            console.log("TSDVJS: drawGraph function is not defined for Graph object!")
        }; //TODO add a default graph draw
        this.plotItems = {};
        this.zoom = args.zoom;
        this.zoomDataSize = args.zoomDataSize;
        this.barSize = 0;
        ...
        this.loadFullStartDate;
        this.loadReducedStartDate;
        this.loadStartTime;
        this.timeToLoadReducedData;
        this.timeToLoadFullData;
    }
    ...
    drawLineGraph = function(thisGraph, isAnimated) {
        ...
        this.lineFunc = d3.svg.line()
            .defined(function(d) {
                return (d[yCoord] !== undefined && d[yCoord] > 0);
            })
            .interpolate("cardinal")
            .tension(1)
            .x(function(d) {
                return xFunc(d[thisGraph.xCoordName]) + (barSize / 2);
            })
            .y(function(d) {
    ...
    */
    TSDVJS.Graph.prototype.addPlotItem = function(selector, name, type, yFunc, customDrawFunc) {
        this.plotItems[name] = new PlotItem();
        this.plotItems[name].selector = selector;
    ...
    */
    TSDVJS.Graph.prototype.removeAllVisibleData = function() {
        ...
        var emptyData = {
            date: parseDate(sqlFormat(currDate[0])),
            steps: 0,
            calories: 0,
            body_temp: 0,
            heart_rate: 0,
        ...
        var emptyData2 = {
            date: parseDate(sqlFormat(currDate[1])),
            steps: 0,
            calories: 0,
            body_temp: 0,
            heart_rate: 0,
        ...
        if (this.zoom) {
            ...
            if (this.zoom.scale() !== 1) {
                ...
                if (this.panning && timeSinceLastZoom < 250) {
                    ...
                    pane.transition().ease("quad-out").duration(800).tween("pan", function() {
